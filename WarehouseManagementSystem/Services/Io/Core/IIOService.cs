...
namespace WarehouseManagementSystem.Service.Io
{
    ...
    public class IOService : IIOService
    {
        ...
        private readonly SemaphoreSlim _globalMonitoringLock = new(1, 1);
        private CancellationTokenSource? _globalMonitoringCts;
        private Task? _globalMonitoringTask;
        ...
        {
            await _globalMonitoringLock.WaitAsync();
            try
            {
                if (_globalMonitoringTask != null && !_globalMonitoringTask.IsCompleted)
                {
                    _logger.LogInformation("IO 全局监控任务已在运行，跳过重复启动");
                    return;
                }

                _logger.LogInformation("开始监控 IO 设备");
                _globalMonitoringCts = new CancellationTokenSource();
                _globalMonitoringTask = Task.Run(() => RunGlobalDeviceMonitoringAsync(_globalMonitoringCts.Token));
            }
            finally
            {
                _globalMonitoringLock.Release();
            }
        }

        public async Task StopDeviceMonitoring()
        {
            await _globalMonitoringLock.WaitAsync();
            try
            {
                if (_globalMonitoringCts == null)
                {
                    return;
                }

                _globalMonitoringCts.Cancel();

                if (_globalMonitoringTask != null)
                {
                    try
                    {
                        await _globalMonitoringTask;
                    }
                ...
                _globalMonitoringCts.Dispose();
                _globalMonitoringCts = null;
                _globalMonitoringTask = null;
                _logger.LogInformation("IO 全局监控任务已停止");
            }
            finally
            {
                _globalMonitoringLock.Release();
        ...
        private void StartMonitoringDevice(RCS_IODevices device)
        {
            ...
            if (_monitoringTasks.ContainsKey(device.IP))
            {
                _logger.LogDebug("设备 {DeviceId} 的监控任务已存在，跳过重复启动", device.Id);
                return;
            }
        ...
        private async Task RunGlobalDeviceMonitoringAsync(CancellationToken cancellationToken)
        {
            _logger.LogInformation("IO 全局监控循环已启动");
            ...
            {
                _logger.LogInformation("IO 全局监控循环已停止");
            }
        }
    }
}
