    ...
    public async Task<bool> CheckDuplicateTask(string sourcePosition, string targetPosition)
    {
        try
        {
            ...
            else
            {
                ...
                parameters.Add("@FinishedFlag", (int)TaskStatuEnum.TaskFinish);
                parameters.Add("@CancelFlag", (int)TaskStatuEnum.Canceled);
            }
    ...
    public async Task<(bool success, string message)> CancelTask(int id)
    {
        try
        {
            ...
            else
            {
                ...
                if (((int)task.taskStatus & (int)TaskStatuEnum.TaskFinish) != 0)
                {
                    return (false, "已完成的任务不能取消");
                }

                // Check if task is already canceled
                if (((int)task.taskStatus & (int)TaskStatuEnum.Canceled) != 0)
                {
                    return (false, "任务已经被取消");
                }

                // Set the Cancel flag
                var newStatus = (int)task.taskStatus | (int)TaskStatuEnum.Canceled;
