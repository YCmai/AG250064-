            ...
            if (BytesLength >= 2)
            {
                ...
                if (func == FunctionCode.Normal)
                {
                    if (BytesLength >= 6)
                    {
                        ...
                        switch (mType)
                        {
                            case MessageType.OrderAcknowledge: //����ȷ��
                                {
                                    ...
                                    switch (status)
                                    {
                                        case AckStatusCode.OrderRejected:
                                            // |= ����˼��group�Ǿ��Ӽ���AckGroupCode��ģ�� ��|�ǰ�λ��&�ǰ�λ��
                            ...
                            case MessageType.OrderEvent://�����¼�
                                ...
                                break;
                            case MessageType.OrderStatus://����״̬
                                {
                                    int mix = -1, tmpc = 0;
                                    ...
                                    if (tmpc < 0x10)
                                    {
                                        ...
                                        oix = DataTransform.BytesToUInt16(BytesData, StaticLength + 4);//��Э�����ݽṹ�������ǰ���λ�ڵ�5���ֽڣ��Ǿ�����6��4��6�ǹ̶���4�Ļ��õ�ǰ�ֽڵ��1
                                        ...
                                        data = new OrderStatusAciData()
                                        {
                                            ...
                                            ErrorCode = errc,               //�������
                                            OrderIndex = oix,               //���������е�����
                                            OrderStartTime = DateTime.Parse("1970/01/01").AddSeconds(stime),//���ʼʱ��
                                            InitialStructure = strp,        //����ṹ���������õ�ǰ����
                                            CurrentStructure = ctrp,        //������ǰ����ṹ
                                            CurrentRow = crow,              //������ǰ����ṹ��
                                            OrderList = olist,              //�����б������
                                            OrderState = ostate,            //����״̬
                                            OrderStatus = ostatus,          //�˶���ִ��״̬
                                            OrderPriority = pri,            //�������ȼ�
                                            OrderTrigger = trig,            //�������
                                            TriggerParam1 = trip1,          //������������1
                                            TriggerParam2 = trip2,          //������������2
                                            CarrierID = cid,                //AGVID
                                            MainStatus = mainstat,          //������Ҫ״̬
                                            PlcStatus = plcs,               //PLC״̬
                                            MoveState = movestat,           //�ƶ�״̬
                                            PreviousStation = pstn,         //���䵱ǰվ��
                                            FinalDestination = dstn,        //Ŀ�ĵ�վ��
                                        };
                                    }
                                    else
                                    {
                                ...
                                break;
                            case MessageType.LocalParamContents://���ز���
                                {
                                    int index;
                            ...
                            case MessageType.LocalParamRequest:
                                ...
                                break;
                            case MessageType.GlobalParamStatus://ȫ�ֲ���״̬
                                {
                                    int mix, pnum, pix;
                                    ...
                                    pnum = DataTransform.BytesToUInt8(BytesData, StaticLength + 3);//���ʵĲ�������
                                    pix = DataTransform.BytesToUInt16(BytesData, StaticLength + 4);//��Ϣλ��P0�е�ȫ�ֲ�����
                            ...
                            case MessageType.AgvOmPlcToHost:
                                //��id,   magic, ƫ����,����ֽ��Ǵ�0��ʼ����ȡ����
                                int carId, magic;
                                OmPlcCommandCode codes = OmPlcCommandCode.None;//������
                                int[] pval;//����
                                ...
                                int pnum1 = (BytesLength - StaticLength - 10) / 2;  //��ȡҪ��ȡ���ֽ���
                ...
                case FunctionCode.Normal: //����
                    {
                        ...
                        switch (gdata.DataType)
                        {
                            case MessageType.OrderInitiate:
                                {
                                    ...
                                    DataTransform.UInt8ToBytes(data.StructureID, msg, 0);                  //����ṹ 
                                    DataTransform.UInt8ToBytes(data.OrderPriority + 0x80, msg, 1);         //���ȼ�
                                    DataTransform.UInt16ToBytes(0x0001, msg, 2);                           //������
                                    DataTransform.UInt16ToBytes(data.OrderKey, msg, 4);                    //Key

                                    for (int i = 0; i < pnum; i++)
                                    {
                                        DataTransform.UInt16ToBytes(data.ParamValues[i], msg, 6 + i * 2);  //����ֵ
                                    }
                                }
                                break;
                            case MessageType.OrderRequest:
                                {
                                    ...
                                    switch (data.ItemCode) //������
                                    {
                                        ...
                                        case OrderItemCode.UsedIndex:
                                            {
                                                msg = new byte[12]; //��ӦЭ��A��������

                                                DataTransform.UInt16ToBytes(data.MagicIndex, msg, 0);           //����
                                                DataTransform.UInt8ToBytes((int)data.ItemCode, msg, 3);         //����״̬����
                                                DataTransform.UInt16ToBytes(data.ItemOffset, msg, 4);           //��Ŀƫ��
                                                DataTransform.UInt16ToBytes(data.ItemNumber, msg, 6);           //����Ŀ���������Ŀ��
                                        ...
                                        case OrderItemCode.ExternalTrigger:
                                            {
                                                ...
                                                DataTransform.UInt16ToBytes(data.MagicIndex, msg, 0);          //����
                                                DataTransform.UInt8ToBytes(data.MagicIndex, msg, 3);           //����״̬����
                                                DataTransform.UInt16ToBytes(data.ItemOffset, msg, 4);          //��Ŀƫ��
                                                DataTransform.UInt16ToBytes(data.ItemNumber, msg, 6);          //����Ŀ���������Ŀ��
                                                DataTransform.UInt32ToBytes(data.ParamFlag, msg, 8);           //������־
                                                DataTransform.UInt16ToBytes((int)data.UnitType, msg, 12);      //IO��Ԫ����
                                                DataTransform.UInt16ToBytes(data.LogicalLine, msg, 14);        //�߼���
                                                DataTransform.UInt16ToBytes(data.LogicalUnit, msg, 16);        //��ԪID��
                                            }
                                            break;
                            ...
                            case MessageType.HostToAgvOmPlc:
                                ...
                                if (datas.MagicIndex > 0xFFFF) { datas.DecompileErrorCode = 6; break; } //��Ϣħ�����ܳ���65535
                                if (datas.ParamCarId < 1 || datas.ParamCarId > 100) { datas.DecompileErrorCode = 14; break; }//agcid
                                if (datas.ParamIndex < 0 || datas.ParamIndex > 239) { datas.DecompileErrorCode = 15; break; ; } //plcƫ����
                                if (datas.ParamNumber < 1 || datas.ParamNumber > 240) { datas.DecompileErrorCode = 16; break; } //��ȡ���������ܳ���240
                                    ...
                                    case OmPlcCommandCode.ReadMultiByte:
                                        {
                                            ...
                                            //DataTransform.UInt8ToBytes(0, msg, 3); //������
